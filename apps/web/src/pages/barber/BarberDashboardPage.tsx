...
// How many days to show at once in the date strip.
const WINDOW_SIZE = 7;

// Navigate this many days back/forward per arrow click.
const STEP = 7;

// Range: 14 days into the past, 21 days into the future (35 total).
// Index PAST_DAYS → today.
const PAST_DAYS = 14;
...
export function BarberDashboardPage() {
  ...
  // selectedDate tracks the calendar day the barber is viewing.
  // windowStart is the index in dateRange of the first visible date.
  const [selectedDate, setSelectedDate] = useState<Date>(
    startOfDay(new Date())
  );
  // Default window starts at "today minus 1" so today is the 2nd visible date
  // and there's already one past day visible without pressing back.
  const [windowStart, setWindowStart] = useState(
    Math.max(0, PAST_DAYS - 1)
  );
  ...
  const { data: appointmentsData, isLoading: loadingAppointments } = useQuery({
    ...
    queryFn: () =>
      appointmentsApi
        .list({ date: selectedDateStr, limit: 50 })
        .then((r) => r.data),
    enabled: !!barberId,
  });

  // Monthly earnings — always current month regardless of selected date.
  ...
  return (
    <div className="page-container animate-fade-in">
      ...
      {/* ── Stats (reflect selected date, except monthly earnings) ── */}
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex gap-1">
            <button
              onClick={() =>
                setWindowStart((prev) => Math.max(0, prev - STEP))
              }
              disabled={!canGoBack}
            ...
            <button
              onClick={() =>
                setWindowStart((prev) =>
                  Math.min(dateRange.length - WINDOW_SIZE, prev + STEP)
                )
              }
